...
## 🔄 3. 即時資料運行機制（SSE / POLLING）

前端資料傳輸採「SSE 優先、Polling 保底」策略：

- 正常情況顯示 `SSE LIVE`：
  - 前端透過 `/stream` 接收即時事件推播，延遲較低。
- 異常或靜默時顯示 `POLLING`：
  - 若 SSE 中斷，或連上但長時間無事件，前端會自動改抓 `/metrics`，避免畫面空白。
- 自動回切：
  - SSE 恢復並收到事件後，會自動停止 polling 並切回 `SSE LIVE`。

> `POLLING` 為容錯保護行為，不一定代表系統故障；重點是資料是否持續更新。

---

## 📊 4. 監控矩陣顯示策略

- 伺服器監控矩陣改為每頁 100 台分頁顯示（避免高密度渲染卡頓）。
- 支援上一頁 / 下一頁與頁碼顯示。
- 分頁文字支援中英文切換。
...
### Q5: 為什麼右上角狀態顯示 `POLLING`？
- **原因**：SSE 連線異常，或 SSE 長時間沒有收到事件，前端啟用保底模式。
- **解法**：先觀察畫面資料是否仍持續更新；若有更新屬正常容錯。若完全不更新，再檢查後端 `/stream` 與 `/metrics` 是否可連通。
