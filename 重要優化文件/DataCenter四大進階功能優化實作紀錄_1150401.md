...
**日期：** 2026-04-21  
**定位：** 歷史優化紀錄，保留作為功能演進參考，不作為目前正式規格文件。

---

## 一、SSE 即時推送 (Server-Sent Events)

> 現況補充：SSE 仍為正式即時主幹，但實際前端為 `SSE 優先、Polling 保底`，相關正式描述請以 `文件/` 內架構文件為準。
...
> 現況補充：液冷與冷卻資料屬重要擴充方向，但部分欄位、閾值與資料來源仍應以當前程式碼實作為準，不宜將本紀錄視為唯一正式規格。
...
> 現況補充：控制與 interlock 的正式行為請以後端實作與目前 API 為準；本節保留作為當時設計方向與優化紀錄。
...
> 現況補充：認證與授權能力已屬正式架構的一部分，但帳號、憑證、Header 與操作方式不應直接以本文件中的示例值作為正式部署設定。
...
> 現況補充：啟動腳本內容可能隨版本調整，請以目前實際 `start.bat` 為準。
...
> **備忘：** 此檔案為歷史優化紀錄；若要看正式現況，請優先閱讀 `文件/` 目錄中的 `1150325` 系列架構文件。
