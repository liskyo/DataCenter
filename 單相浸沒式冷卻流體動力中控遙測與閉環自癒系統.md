...
## 五、 閉環維護與 AI 預測派單自癒機制 (Closed-Loop Self-Healing)

為了將預測性遙測數據與「維護排程系統」深度整合，實現閉環防護，系統新增了自動化派單與耗材自癒清零機制：

### 1. 故障自動派單防重機制 (Automated Dispatch)
後端引擎在運作中，若預測出濾清器剩餘壽命低於限值，會自動向 `MaintenanceService` 派發維修工單。
* **AI 標記與防重檢驗**：自動建立的工單會被標記 `is_auto_generated=True`。在生成前，系統會檢索當前未結案（`status != "COMPLETED"`）的工單，避免重覆發單造成管理混亂。

### 2. 閉環自癒清零 (Closed-Loop Reset)
當工程師在前端排程面板將工單標記「結案」時，會透過 POST 呼叫以下 Endpoint：
`POST /api/maintenance/schedules/{schedule_id}/complete`
這項操作會觸發 `reset_consumables` 函式，執行以下物理重置：
* **清除模擬器故障覆寫**：將對應的模擬器故障碼（如 `clogged_filter`、`water_intrusion` 等）設為 `False`。
* **重置內部歷史時序**：清空引擎內部的壓差與液位歷史資料（`_dp_history`、`_level_history`），使壽命預測演算法重新開始計時。
* **恢復出廠安全數值**：將遙測緩存中最接近的物理讀數（如單相壓差重置為 `2.2 PSI`、介電強度 `50.0 kV`，雙相液位 `480.0 mm`、電導率 `0.08 uS/cm` 等）恢復至安全起始點，達成物理與遙測閉環。

### 3. 前端 UI 聯動設計
* **AI 標籤視覺提示**：前端 `MaintenanceScheduleCard` 將針對 `is_auto_generated` 的工單渲染帶有呼吸燈動畫的紫色 `🤖 AI 預測派單` 專屬徽章。
* **核心資料防篡改鎖定**：當編輯 AI 自動生成的工單時，`MaintenanceForm` 會偵測並鎖定 `設備名稱`、`設備類別` 與 `維護項目` 欄位為唯讀（Disabled），防止人員誤操作損害系統完整性。
* **一鍵結案按鈕**：為未結案的工單提供 `結案 (Complete)` 按鈕，使工程師能以極高體驗完成一鍵自癒調度。
