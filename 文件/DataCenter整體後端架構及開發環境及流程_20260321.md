...
### 4. 系統健康度與維運監控 (Health & Debug Endpoints)
除了主業務邏輯，API 層更實作了維運監控：
- `GET /health`：向 K8s 或外部探針匯報大腦存活狀態。
- `GET /debug`：主動揭露內部環境狀態 (如 Current System Mode、Kafka 啟動狀態、快取承載數量)，非常便於 DevOps 工程師直接除錯。
- **防呆與斷點保障**：背景的 `simulation_worker` 在運作時若發生無法預期的崩潰，會立即捕捉例外並自動寫入背景端的 `sim_error.log`，確保背景守護執行緒不會「死得不明不白」。此外我們已將模擬週期大幅縮短至每 20 秒一次更迭，確保前端視覺展示擁有極高敏感度。
