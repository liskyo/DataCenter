...
export default function EngineeringPage() {
  ...
  const handleAuth = () => {
    ...
    if (code === "admin" || code === "kernel123") {
      ...
      setError("ACCESS DENIED: Invalid Override Code");
      setCode("");
    }
  };
       ...
       <header className={`mb-6 flex items-center gap-4 ${isAuth ? 'bg-emerald-950/30 border-emerald-900' : 'bg-[#0a1e3f]/30 border-rose-900'} p-4 rounded-xl border`}>
        ...
        <div>
          ...
          <p className={`${isAuth ? 'text-emerald-900' : 'text-rose-900'} text-xs font-mono tracking-widest mt-1 font-bold`}>
            {isAuth ? "STATUS: KERNEL ACCESS GRANTED" : "WARNING: KERNEL LEVEL ACCESS"}
          </p>
        </div>
      </header>
      ...
      <div className={`flex-1 flex items-center justify-center p-6 border ${isAuth ? 'border-emerald-900/50 bg-[repeating-linear-gradient(45deg,rgba(16,185,129,0.05),rgba(16,185,129,0.05)_10px,transparent_10px,transparent_20px)]' : 'border-rose-900/50 bg-[repeating-linear-gradient(45deg,rgba(159,18,57,0.05),rgba(159,18,57,0.05)_10px,transparent_10px,transparent_20px)]'} relative`}>
         ...
         {!isAuth ? (
           ...
           <div className="relative z-10 flex flex-col items-center max-w-lg text-center bg-[#050510] border-2 border-rose-500 p-8 shadow-[0_0_30px_rgba(244,63,94,0.3)]">
              ...
              <h2 className="text-2xl font-black text-white tracking-widest mb-4">RESTRICTED AREA</h2>
              <p className="text-slate-400 text-sm mb-6 leading-relaxed">
                This module provides direct memory access (DMA) to the sensor firmware array. 
                Unauthorized modifications may cause catastrophic equipment failure.
              </p>
              ...
              <div className="mt-8 flex items-center justify-center gap-2 text-rose-900 text-xs font-mono">
                <TerminalSquare size={14}/> TERMINAL LOCKED (Hint: admin)
              </div>
           </div>
         ) : (
           // 解鎖狀態 UI
           <div className="relative z-10 w-full max-w-4xl bg-[#050510] border-2 border-emerald-500 p-8 shadow-[0_0_30px_rgba(16,185,129,0.2)]">
              <div className="flex items-center justify-between mb-8 border-b border-emerald-900 pb-4">
                 <div className="flex items-center gap-3">
                   ...
                   <div>
                      <h2 className="text-xl font-black text-white tracking-widest">ROOT CONSOLE</h2>
                      <p className="text-emerald-600 font-mono text-xs">Session Active: Admin</p>
                   </div>
                 </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                 ...
                 <div className="border border-emerald-900 p-4">
                    <h3 className="text-emerald-500 font-bold tracking-widest font-mono mb-4 flex items-center gap-2"><Cpu size={16}/> SENSOR ARRAY CALIBRATION</h3>
                    <div className="space-y-4">
                       <div className="flex justify-between text-slate-300 text-sm"><span>Temperature Bias Offset</span> <span className="text-emerald-400">-0.5°C</span></div>
                       <div className="flex justify-between text-slate-300 text-sm"><span>CPU Utilization Poll Rate</span> <span className="text-emerald-400">1000ms</span></div>
                       <button className="w-full mt-4 bg-emerald-950/50 border border-emerald-800 text-emerald-400 py-2 font-mono text-xs hover:bg-emerald-900">APPLY CALIBRATION</button>
                    </div>
                 </div>

                 {/* 緩存清理 */}
                 <div className="border border-emerald-900 p-4">
                    <h3 className="text-emerald-500 font-bold tracking-widest font-mono mb-4 flex items-center gap-2"><DatabaseZap size={16}/> CACHE FLUSH UTILITY</h3>
                    <div className="space-y-4">
                       <p className="text-slate-400 text-xs">Forces immediate write-ahead log (WAL) sync to MongoDB and flushes Redis in-memory cache.</p>
                       <button className="w-full mt-4 bg-rose-950/50 border border-rose-800 text-rose-400 py-2 font-mono text-xs hover:bg-rose-900 transition-colors">FLUSH SYSTEM CACHE</button>
                    </div>
                 </div>
              </div>
           </div>
